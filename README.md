...
| モード | 入力 | ネット通信 | 用途 |
|---|---|---|---|
| **貼り付け（推奨）** | 検索結果をコピペ | 不要（座標が本文にあれば）／地名の座標化のみ Nominatim | note/Twitter の WAF を回避。確実に動く |
| 取得 | 作品名 | note(+Twitter API) を自動取得 | 通信が許可された環境向け |

> WAF や許可リストで自動取得が 403 になる環境では、**ブラウザでコピーした検索結果を
> 貼り付ける「貼り付けモード」**が確実です。本文に Google Map リンク/座標が含まれていれば
> ネット通信ゼロで座標化でき、無ければ抽出した地名だけ Nominatim で座標化します。
